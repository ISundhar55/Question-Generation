...
const generateQuestions = async (req, res) => {
  try {
    ...
    const validTypes = ['SINGLE_SELECT', 'MULTIPLE_SELECT', 'MCQ', 'TRUE_FALSE', 'CONSTRUCTED_RESPONSE', 'DROPDOWN', 'MATCHING_LINES', 'ORDERING', 'BACKGROUND_GRAPHIC', 'GAP_MATCH', 'MULTIPLE_DROP_BUCKET', 'MATRIX_INTERACTION', 'SELECT_TEXT'];
...
async function generateFromInternet(req, res) {
  try {
    ...
    // Proxy to Python RAG service — internet generation endpoint
    let pyRes;
    try {
      pyRes = await fetch(`${PYTHON_SERVICE}/generate-internet`, {
        method: 'POST',
        headers: internalHeaders(),
        body: JSON.stringify({
          content_area,
    ...
    console.error('[aiController] generateFromInternet error:', err);
